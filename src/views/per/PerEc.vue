  ...
  <div>
    ...
    <div>
      <el-table :data="PerEcs" border style="width: 85%">
        ...
        <el-table-column prop="employee.username" label="员工姓名" width="150">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 请求员工信息
    loadAllEmp() {
      this.getRequest("/personnel/emp/").then(resp => {
        if (resp) {
    ...
    initRuleRole() {
      ...
      if (this.search.empname && this.search.empname != null) {
        url += "&username=" + this.search.empname;
      }
      if (this.search.userID && this.search.userID != null) {
        url += "&userID=" + this.search.userID;
      }
    ...
    showView(row) {
      if (this.validateIt == 0) {
        ...
        this.PerEc.userID = row.employee.userID;
        this.PerEc.empname = row.employee.username;
    ...
    onSubmit() {
      if (this.validateIt == 0) {
        // 查询
        this.search.userID = this.PerEc.userID;
