...
<template>
  <div>
    ...
    <!-- 数据显示区域 -->
    <div>
      <el-table :data="PerTrains" size="large" border style="width: 79%">
...
export default {
  ...
  methods: {
    ...
    handleSelectEmp(item) {
      ...
      this.PerTrain.userID = item.userID;
    },
    //
    searchItem() {
    ...
    editItem(row) {
      ...
      this.PerTrain.userID = this.PrefixInteger(row.eid, 8);
      this.PerTrain.username = row.employee.username;
    ...
    onSubmit() {
      if (this.validateIt == 0) {
        this.search.username = this.PerTrain.username;
